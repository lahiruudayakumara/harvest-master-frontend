...
import { Button, Dialog, DialogTitle, DialogContent, DialogActions, Typography, TextField, Box, ThemeProvider, createTheme } from "@mui/material";
...
const PopupDialogSupport = (props) => {
 ...
 // Regular expression to match only letters, spaces, and one optional full stop at the end
  ...
  const handleSubmit = async () => {
    const response = await updateSupportSolution(props.data.r_Id,props.data.topic,props.data.issue,solution);
  ...
  return (
    <ThemeProvider theme={theme}>
      <Box width={500}>
        ...
        <Dialog open={open} onClose={handleClose} sx={{  mt: 10 ,width:"100%"}} maxWidth={"sm"} >
          ...
          <DialogActions>
            ...
            <Button onClick={handleSubmit} variant="contained" color="primary" disabled={solution == null||solution==""}>Submit</Button>
          </DialogActions>
        </Dialog>
      </Box>
    </ThemeProvider>
